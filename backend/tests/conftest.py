...
async def _ensure_chat_session_columns(engine) -> None:
    async with engine.begin() as conn:
        await conn.execute(
            text("ALTER TABLE chat_sessions ADD COLUMN IF NOT EXISTS session_id VARCHAR(255)")
        )
        ...
        await conn.execute(
            text("ALTER TABLE chat_sessions ADD COLUMN IF NOT EXISTS last_activity_at TIMESTAMPTZ")
        )
        await conn.execute(
            text(
        ...
        await conn.execute(
            text("ALTER TABLE chat_sessions ALTER COLUMN last_activity_at SET DEFAULT NOW()")
        )
        await conn.execute(
        ...
        await conn.execute(
            text("ALTER TABLE goals ADD COLUMN IF NOT EXISTS awaiting_direction_id VARCHAR(255)")
        )
