...
def test_script_exists_and_is_executable() -> None:
    ...
    assert os.access(SCRIPT_PATH, os.X_OK), "scheduled-camera-permission-audit.sh must be executable"
...
def test_dry_run_prints_default_target_and_playwright_command() -> None:
    ...
    assert "E2E_BASE_URL=http://localhost:8083/?uxAuditScenario=camera-permission-denied" in output
    assert "E2E_API_URL=http://localhost:8001" in output
    assert "npx playwright test e2e/audit_camera_permission_denied.spec.ts --project=chromium" in output
...
def test_dry_run_honors_overridden_audit_urls() -> None:
    ...
    assert "E2E_BASE_URL=https://audit.example/?uxAuditScenario=camera-permission-denied" in output
