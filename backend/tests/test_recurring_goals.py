...
async def _create_active_recurring_goal(client, token, recurrence="daily", deadline_delta_days=1):
...
async def test_recurring_weekly_goal_creates_new_instance():
    ...
    async with make_client() as client:
        token, user = await _auth(client)
        goal_id = await _create_active_recurring_goal(
            client, token, recurrence="weekly", deadline_delta_days=7
        )
...
async def test_recurring_monthly_goal_creates_new_instance():
    ...
    async with make_client() as client:
        ...
        deadline_30days_ago = datetime.now(UTC) - timedelta(days=30)
        resp = await client.post(
            "/api/goals",
            headers={"Authorization": f"Bearer {token}"},
            json={
...
async def test_recurring_goal_creates_notification_for_new_instance():
    ...
    async with make_client() as client:
        token, user = await _auth(client)
        await _create_active_recurring_goal(client, token, recurrence="daily")
        ...
        instance_notifications = [
            n for n in notifications if n.type == "goal_created" and "Recurring" in n.title
        ]
