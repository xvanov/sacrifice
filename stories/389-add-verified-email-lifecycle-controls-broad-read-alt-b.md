...
- **Task 5 (Verify-request):** Generates cryptographically-random token (`secrets.token_urlsafe(32)`), stores SHA-256 hash in VerificationToken with 15-min expiry. Returns plaintext token only when `config.ENVIRONMENT != "production"`. Rejects already-verified accounts with 409. Rate-limited with per-user 60s cooldown.
